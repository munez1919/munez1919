# munez1919
# 👋 Hi! Меня зовут Вячеслав! ;)

🔹 **Интересы:** DevOps, Linux, PostgreSQL, Kubernetes, Docker, CI/CD  
🔹 **Цель:** Стать DevOps-инженером и работать над масштабными проектами  
🔹 **Изучаю:** Ansible, Zabbix, Nginx, Jenkins, Kubernetes, Git/GitHub  

## 🛠 Навыки и технологии:
- **Языки:** Bash, Python, SQL  
- **DevOps-инструменты:** Docker, Kubernetes, Jenkins, GitHub Actions  
- **Базы данных:** PostgreSQL  
- **ОС:** Linux (Kali, Ubuntu, CentOS)  
- **Мониторинг:** Zabbix  

## 📌 Мои проекты:
- 🔹 **Мониторинг серверов + Телеграм-уведомления**  
  📌 **Технологии:** Linux, PostgreSQL, Docker, Jenkins, CI/CD, Kubernetes, Telegram API  
  📌 **Описание:** Проект по мониторингу серверов с отправкой уведомлений в Telegram.  

## 📊 GitHub Статистика:
![munez1919's GitHub Stats](https://github-readme-stats.vercel.app/api?username=munez1919&show_icons=true&theme=radical)

[![GitHub](https://img.shields.io/badge/GitHub-Profile-black?style=flat&logo=github)](https://github.com/munez1919)

